...
func (tx *TxGraph) CreatePolicyClass(name string, properties graph.PropertyMap) (*graph.Node, error) {
    ...
    cmds := tx.cmds["create_policy_class"]
    cmds = append(cmds, &txGraphCommitterImpl{
        c: func() error {
...
func (tx *TxGraph) Children(name string) set.Set {
    ...
    cmds := tx.cmds["deassign"]
    for _, txCmd := range cmds {
...
func (tx *TxGraph) Deassign(child, parent string) error {
    ...
    cmds := tx.cmds["deassign"]
    cmds = append(cmds, &txGraphDeassignCommitter{
        c: func() error {
...
func (tx *TxGraph) Dissociate(ua, target string) error {
    ...
    cmds := tx.cmds["dissociate"]
    cmds = append(cmds, &txGraphDissociateCommitter{
        c: func() error {
